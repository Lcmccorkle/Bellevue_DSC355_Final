...
def load_model_and_reference_data():
    """
    Loads the trained XGBoost model and (optionally) the reference dataset 
    for realistic dropdown choices and default values.
    """
    ...
    try:
        ...
        st.error("→ Make sure you saved it from your Milestone 4 notebook.")
        st.stop()

    # Try to load reference data for realistic choices
    df_ref = None
    try:
        df_ref = pd.read_csv('engineered_walmart_data_Part1.csv', 'engineered_walmart_data_Part2.csv', 'engineered_walmart_data_Part3.csv','engineered_walmart_data_Part4.csv', parse_dates=['Date'])
    except FileNotFoundError:
        st.warning("engineered_walmart_data.csv not found → using fallback defaults.")
...
if df_ref is not None:
    ...
    types  = ['A', 'B', 'C']  # or derive from df_ref if 'Type' exists
else:
    # Fallback ranges if data file is missing
    stores = list(range(1, 46))
    depts  = list(range(1, 100))
    types  = ['A', 'B', 'C']

# Sensible default values (approximate medians / common values)
DEFAULTS = {
    'size': 140000,
    'temperature': 60.0,
    'fuel_price': 3.3,
...
st.markdown("""
Predict **weekly department-level sales** using the XGBoost model from Milestone 4.  
Enter the conditions → get a dollar estimate.
""")

st.info("Top SHAP features: Store, Dept, Size, IsHoliday, Temperature, Markdowns, Type, CPI, Unemployment")

# ── Input Form ───────────────────────────────────────────────────────────────
with st.form("sales_prediction_form"):

    # Section 1: Store & Department
    ...
    # Section 3: Store & Economic Features
...
# ── Make Prediction when submitted ───────────────────────────────────────────
if submit:
    # Build feature dictionary matching training columns
    input_dict = {
        'Store': store,
        'Dept': dept,
        ...
        # Add any other engineered features your model expects, e.g.:
    ...
    # Try to match exact column order the model was trained on
    try:
        expected_cols = model.feature_names_in_
        input_df = input_df[expected_cols]
    except AttributeError:
        st.warning("Could not read model feature names — hoping column order matches.")

    # Predict
    log_pred = model.predict(input_df)[0]
    dollar_pred = np.expm1(log_pred)   # Use np.exp() instead if you used plain np.log()

    # Display results
    st.success("Prediction ready!")
    st.metric(label="Predicted Weekly Sales", value=f"${dollar_pred:,.0f}")
    st.metric(label="(Log-scale internal)", value=f"{log_pred:.4f}")
...
with st.expander("Model Summary (Milestone 4)"):
    st.markdown("""
    - **Model**: XGBoost Regressor (tuned + early stopping)
    - **Target**: log(Weekly_Sales + 1) → transformed back to dollars
    - **Test performance (approx)**: MAE $7k–$9k, R² 0.93–0.96
